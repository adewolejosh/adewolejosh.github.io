            ...
            <div v-if="article">
                ...
                <div class="mt-5">
                    <div class="mt-5 mb-2">
                        ...
                        <div class="mt-3" v-if="article.meta?.tags">
                            <button v-for="(tag, id) in article.meta.tags" :key="id"
                    ...
                    <hr />
                    <img v-if="article.meta?.img" class="d-flex mx-auto mt-5 mb-5 img-fluid"
                        :src="`/images/blog/${article.meta.img}`" :alt="article.meta?.alt || article.title"
                        id="post-img" />

                    <div v-if="article.meta?.quote">
                        <p class="font-weight-lighter post-quote me-3 ms-3 text-center">{{ article.meta.quote }}</p>
                        <p v-if="article.meta?.ref" class="font-weight-lighter post-ref text-center">{{ article.meta.ref
                            }}</p>
                    </div>

                    <div class="nuxt-content">
                        <ContentRenderer class="text-justify lh-lg tracking-wider ignore-css" :value="article" />
                    </div>

                    <hr class="mt-5" />
                    <div class="footer-copyright text-center">© (20)21-26 Copyright:
                        <a target="_blank" class="text-dark text-decoration-none"
                            href="mailto:adewole.josh@gmail.com">Joshua Adewole</a>
                        <a target="_blank" class="text-dark text-decoration-none"
                            href="https://github.com/adewolejosh">(adewolejosh)</a>
                    </div>
...
const { data: article } = await useAsyncData(`blog-${slug.value}`, async () => {
...
// Set page meta
useHead({
    title: article.value?.title ? `Josh's blog: ${article.value.title}` : "Josh's blog",
    meta: [
        {
            name: 'description',
